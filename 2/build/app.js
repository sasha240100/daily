...
exports.rbModule = exports.controls = exports.app = exports.controlsModule = undefined;

var _camera = __webpack_require__(36);

var _modules = __webpack_require__(17);

var _RenderBufferModule = __webpack_require__(59);

var controlsModule = exports.controlsModule = new WHS.OrbitControlsModule({
  object: _camera.camera
});

var app = exports.app = new WHS.App([new WHS.ElementModule(), new WHS.SceneModule(), new WHS.DefineModule('camera', _camera.orthoCamera), // orthoCamera
_modules.renderingModule, controlsModule, _modules.resizeModule]);
...
_modules.resizeModule.camera = _camera.camera;

var getSize = function getSize() {
  return _modules.renderingModule.renderer.getSize();
};
var size = getSize();

var rbModule = exports.rbModule = new _RenderBufferModule.RenderBufferModule(size.width, size.height, {
  helper:  true ? null : new FBOHelper(_modules.renderingModule.renderer),
  scene: app.get('scene'),
  camera: _camera.camera.native,
  fixCamera: false
});
...
rbModule.defineBuffers([rbModule.createRT(options), rbModule.createRT(options)]);

_modules.resizeModule.addCallback(function () {
...
var resizeModule = exports.resizeModule = new WHS.ResizeModule();
var size = exports.size = renderingModule.renderer.getSize();
...
exports.orthoCamera = exports.camera = undefined;

var _modules = __webpack_require__(17);

var _renderingModule$rend = _modules.renderingModule.renderer.getSize(),
    w = _renderingModule$rend.width,
    h = _renderingModule$rend.height;

var camera = exports.camera = new WHS.PerspectiveCamera({
  position: [0, 0, 100]
});

var orthoCamera = exports.orthoCamera = new WHS.OrthographicCamera({
  left: -w / 2,
  right: w / 2,
  top: -h / 2,
  bottom: h / 2,
...
var _PointGrid = __webpack_require__(88);

var _app = __webpack_require__(10);

var _grid = __webpack_require__(114);

var grid = exports.grid = new _PointGrid.PointGrid({
  geometry: [100, 100, 30, 30],
  material: _grid.material
});
...
var projection = exports.projection = new WHS.Plane({
  geometry: [_modules.size.width, _modules.size.height],
...
var RenderBufferModule = function () {
  function RenderBufferModule(width, height, _ref) {
    var _ref$helper = _ref.helper,
        helper = _ref$helper === undefined ? null : _ref$helper,
        _ref$scene = _ref.scene,
        scene = _ref$scene === undefined ? new THREE.Scene() : _ref$scene,
        cameraObject = _ref.camera,
        _ref$fixCamera = _ref.fixCamera,
        fixCamera = _ref$fixCamera === undefined ? true : _ref$fixCamera;
  ...
  (0, _createClass3.default)(RenderBufferModule, [{
    ...
    value: function setSize(width, height) {
      this.buffers.forEach(function (buff) {
        buff.setSize(width, height);
      });
    }
  }, {
    key: 'createRT',
    value: function createRT(options) {
      var _size = this.size,
          width = _size.width,
          height = _size.height;


      return new THREE.WebGLRenderTarget(width, height, (0, _assign2.default)({
    ...
    value: function update(cb) {
      ...
      new WHS.Loop(function () {
        return cb(_this3.data, _this3);
      }).start(this.app);
      if (this.helper) new WHS.Loop(function () {
        return _this3.helper.update();
      }).start(this.app);
    }
  }]);
...
var defaults = (0, _assign2.default)({}, WHS.MeshComponent.defaults, {
  geometry: {
    width: 100,
    height: 100,
    widthSegments: 10,
...
var instructions = (0, _assign2.default)({}, WHS.MeshComponent.instructions, {
  geometry: ['width', 'height', 'widthSegments', 'heightSegments']
});

var PointGrid = exports.PointGrid = function (_WHS$MeshComponent) {
  (0, _inherits3.default)(PointGrid, _WHS$MeshComponent);

  function PointGrid() {
    var params = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    (0, _classCallCheck3.default)(this, PointGrid);
    return (0, _possibleConstructorReturn3.default)(this, (PointGrid.__proto__ || (0, _getPrototypeOf2.default)(PointGrid)).call(this, params, defaults, instructions));
  }

  (0, _createClass3.default)(PointGrid, [{
    key: 'build',
    value: function build(_ref) {
      var _ref$geometry = _ref.geometry,
          width = _ref$geometry.width,
          height = _ref$geometry.height,
          widthSegments = _ref$geometry.widthSegments,
          heightSegments = _ref$geometry.heightSegments,
          material = _ref.material;

      return new THREE.Points(new THREE.PlaneBufferGeometry(width, height, widthSegments, heightSegments), material);
    }
  }]);
  return PointGrid;
}(WHS.MeshComponent);
...
var _grid3 = __webpack_require__(116);
...
var _projection = __webpack_require__(57);

var _clear = __webpack_require__(118);

_app.rbModule.setup(function (_ref) {
  ...
  var clearPlane = new THREE.Mesh(new THREE.PlaneBufferGeometry(size, size, 1, 1), _clear.material
...
var switchVisible = function switchVisible(isActive) {
  return function () {
    for (var _len = arguments.length, meshes = Array(_len), _key = 0; _key < _len; _key++) {
      meshes[_key] = arguments[_key];
    }
...
var activate = switchVisible(true);
var deactivate = switchVisible(false);

_app.rbModule.renderer.render(_app.rbModule.scene, _app.rbModule.camera);

_app.rbModule.update(function (_ref2, self) {
  var clearPlane = _ref2.clearPlane;

  activate(_grid.grid.native, clearPlane);

  deactivate(_projection.projection.native);

  self.renderBuffer(0);

  _clear.material.uniforms.readBuffer.value = self.buffers[0];
  _projection.projection.material.map = self.buffers[0];
  ...
  deactivate(_grid.grid.native);
  activate(_projection.projection.native);
});

window.grid = _grid.grid;

var plane = _app.rbModule.data.clearPlane;

function updatePlaneSize() {
  plane.position.copy(_camera.camera.position.clone().negate());
  plane.lookAt(_camera.camera.position);

  // console.log(camera.native.getEffectiveFOV())
  var aspect = window.innerWidth / window.innerHeight;

  var distance = _camera.camera.position.length();

  plane.scale.set(distance * aspect, distance, 1);
}

_app.controls.addEventListener('change', updatePlaneSize);
_modules.resizeModule.addCallback(function () {
  updatePlaneSize();
});
...
var _clear = __webpack_require__(119);

var _clear2 = _interopRequireDefault(_clear);

var _clear3 = __webpack_require__(120);

var _clear4 = _interopRequireDefault(_clear3);
...
var material = exports.material = new THREE.ShaderMaterial({
  uniforms: {
    readBuffer: { value: _app.rbModule.buffers[0].texture }
  },

  vertexShader: _clear2.default,
  fragmentShader: _clear4.default
});
